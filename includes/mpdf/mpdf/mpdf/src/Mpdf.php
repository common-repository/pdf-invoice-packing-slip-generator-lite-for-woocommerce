...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function SetHTMLHeader($header = '', $OE = '', $write = false)
	{
		...
		if ($this->setAutoTopMargin == 'pad') {
			...
			if( $rtwcpiglw_top_marg ){
				$sgb_top_marg = $rtwcpiglw_top_marg;	
			}else {
				$sgb_top_marg = 16;
			}
	...
	function Footer()
	{
		...
		if (($this->watermarkText) && ($this->showWatermarkText)) {
			$rtwwcfp_get_setting = get_option('rtwcpiglw_enable_text_watermark');
			if($rtwwcfp_get_setting != ''){
		  		$rotate_custom = $rtwwcfp_get_setting;
		  	}else{
		  		$rotate_custom = 45;
		  	}
